...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"dyAr":[function(require,module,exports) {
...
AFRAME.registerComponent('contentfan', {
  ...
  init: function () {
    ...
    this.angleInterval = 0; // Variables required by our animation function
    // Anime.js must animate a property, or class variable

    this.startingRotation = 0;
    this.animationActive = false;
    this.animationDelta = 0; // Add a cylinder geometry
  ...
  buildWithContentElements: function (elements) {
    let self = this; // Build some blades on which we'll show some content

    var numBlades = Math.max(elements.length, 3); // var numBlades = Math.max(elements.length, 3);
    // 2 blades looks dumb
    ...
    elements.forEach(function (el, i) {
      ...
      let dist = self.data.radius + 5; // We create a container so that we can adjust the anchor and pivot point
      ...
      let planeContainer = document.createElement('a-entity'); // let planeContainer = document.createElement('a-plane');
      ...
      let z = -dist * Math.sin(currentAngle); // Scoot plane over so we're positioning it from the edge
      // Hardcoded crap

      el.object3D.position.x = -3.5;
      el.object3D.position.y = 3.5; // Position plane around cylinder
      // planeContainer.object3D.position.x = x - 14

      planeContainer.object3D.position.x = x; //- 6

      planeContainer.object3D.position.z = z; // Rotate so we're mostly facing the camera
      ...
      planeContainer.object3D.rotation.y = currentAngle + THREE.Math.degToRad(0); //+ THREE.Math.degToRad(-360 / numBlades))
      // Save this angle to our "key frames" that we can animate to

      self.contentKeyframes.push(currentAngle); // console.log("keyframe, ", THREE.Math.radToDeg(currentAngle));
    ...
    this.contentKeyframes.reverse(); // Set initial rotation
  ...
  animateToContent: function (index) {
    ...
    if (index >= 0 && index < this.contentKeyframes.length) {
      ...
      var deltaRotation = targetAngle - curRot; // Find delta in range of -PI to PI
      ...
      this.startingRotation = curRot; // Kind of clunky, but to dynamically make an animation...
