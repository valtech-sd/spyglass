...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"ckvF":[function(require,module,exports) {
...
AFRAME.registerComponent('multi-marker-rotation-interpolator', {
  init: function () {
    ...
    this.minRotation = -50;
    this.maxRotation = 50;
    this.isTracking = false; // Find all of the tags in this group

    this.tagElements = this.el.querySelectorAll('a-marker'); // Add adjusted rotation component to tag elements, if it doesn't already exist
  ...
  lastTrackedTag: function () {
    ...
    this.tagElements.forEach(function (tag, index) {
      ...
      if (entity.visible) {
        ...
        lastTrackedElement = tag; // This is our first tracked tag
        ...
        if (this.prevTrackedIndex != lastTrackedIndex) {
          ...
          if (!self.emittedIndexEvent) {
            ...
            if (deltaTimestamp > self.minTrackDuration) {
              self.el.emit("tag-index-trigger", {
                index: lastTrackedIndex
              });
  ...
  tick: function () {
    ...
    if (entity.visible) {
      let lastTrackedInfo = this.lastTrackedTag(); // Figure out last tag visible
      ...
      if (lastTag && this.emittedIndexEvent) {
        ...
        let currentPosition = lastTag.getAttribute("position"); // Compute delta from starting rotation
        // Focus on "flip" rotation about y-axis

        let deltaRotationAboutY = this.initialRotation.y - currentRotation.y; // Compute normalized value of rotation, clamping to min/max rotation

        let clampedRotation = THREE.Math.clamp(deltaRotationAboutY, this.minRotation, this.maxRotation);
        let normalizedRotation = THREE.Math.mapLinear(clampedRotation, this.minRotation, this.maxRotation, 0, 1); // this.el.emit("tag-rotation", { index: lastTrackedInfo.index, normalizedRotation: normalizedRotation });

        this.el.emit("tag-rotation", {
          index: lastTrackedInfo.index,
          rotation: currentRotation
        });
        this.el.emit("tag-position", {
          index: lastTrackedInfo.index,
...
//# sourceMappingURL=/skincare/multi-marker-rotation-interpolator.a2ade3fb.js.map